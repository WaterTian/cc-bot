...
- **Stage E** 「已配置」判定按 IM 分流：
  ...
  - 判定为「已配置」→ 输出「已配置，可 /cc-bot:start」。**仍需复查**（共享，IM 无关）：
    - **Profile 字段 migration（v0.1.28+）**：跑 `node ${CLAUDE_PLUGIN_ROOT}/runtime/profile-migrate.js apply --project <root>`，自动 backfill 历史新增字段（`busy_placeholder` / `busy_reaction` / `streaming_card` / `intent_permissions` / `privacy` 等），不覆盖已设值，幂等可重复。返回 added count > 0 时在终端报「补全 N 个新版字段：[paths...]」
