...
Run `lark-cli auth list` once. Based on the output branch:

**Non-empty JSON array（已登录）**: extract first entry — `appId` → **BOT_APP_ID**, `userOpenId` → **ADMIN_OPEN_ID**, `userName` → **ADMIN_NAME**. Tell user `✓ 已登录 {ADMIN_NAME}（{BOT_APP_ID}）`. Skip to Stage D. (If multiple entries, `AskUserQuestion` to pick which appId.)
...
收到「继续」后请用户跑 `lark-cli auth login`（浏览器 Device Flow）。再次 `lark-cli auth list` 解析 BOT_APP_ID / ADMIN_OPEN_ID / ADMIN_NAME。
...
5. **If N ≥ 4**: fall back to a numbered text list (AskUserQuestion limits options to 4):
   ```
   bot 所在的群：
   1. {名字1} ({chat_id_1})
   2. {名字2} ({chat_id_2})
   ...
   请回复编号选择目标群（如 "1"）。
   ```
   Wait for numeric response. If user picks **N+1** → **D.new**. Otherwise record **CHAT_ID** / **CHAT_NAME** from the picked item.

### Sub-flow D.new — 创建新群

1. Determine default group name: `cc-bot: {current_dir_basename}` (e.g. project dir `D:\Projects\cc-bot-test` → `cc-bot: cc-bot-test`).
2. Tell user: `即将创建新群「{default_name}」并把你加为群主。想改名直接回复新名字，否则回复「确认」继续。`
3. Parse user response:
   - `确认` / `yes` / empty → use default name.
   - Any other text → use that as the name (truncate to 60 chars per lark limit).
4. Run:
   ```bash
